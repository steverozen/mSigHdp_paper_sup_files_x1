  ...
  for (se in all.seeds()) {
    ...
    dd_one_seed_return <- 
      dd_one_seed(dir                 = dx, 
                  ...
                  approach            = approach)
    browser()
    fp.sig.list <- c(dd_one_seed_return$fpsigs, fp.sig.list)
  }
  fplist.catalog <- do.call(cbind, fp.sig.list)
...
dd_one_seed <- function(dir,
  ...
  fpsigs.names <- xx$unmatched.ex.sigs
  fpsigs <- ex[ , fpsigs.names, drop = FALSE]
  ...
  return(list(fpsigs = fpsigs, trow =
                tibble::tibble_row(fpsigs.names = fpsigs.names, 
              recon.neg = can.we.reconstruct, recon.all = can.we.reconstruct2 )))  
}

reconstruct1 <- function(target.sig, sig.universe, cat.fn) {

...
move_sigpro_files_one_dir <- function(a.dir) {
  
  start.here <- file.path(a.folder, "raw_results")

  
  # Check whether a.dir contains SBS or indel data sets.
  # This is useful to call different functions to transfrom SigPro catalog.
  flag_indel <- FALSE
  flag_SBS <- FALSE
  if (grepl("indel", basename(a.dir))) {
    # indel_set1, indel_set2
    # indel_set1_down_samp, indel_set2_down_samp
    flag_indel <- TRUE
  }
  if (grepl("SBS", basename(a.dir))) {
    # SBS_*
    # ROC_SBS35_*
    flag_indel <- FALSE
  }
  
  # Skip a.dir is not a folder with indel or SBS data set.
  if (flag_SBS || flag_indel == FALSE) {
    message("Skipping ", a.dir, "as it is not a folder for SBS or indel data sets")
    invisible(NULL)
  }
  if (flag_SBS && flag_indel == TRUE) {
    stop("a.dir cannot be both an SBS and indel data set\n")
  }
  
  tools <- "SigProfilerExtractor.results"
  for (analysis.name in tools) {
    stopifnot(dir.exists(analysis.name)) # A full directory path, e.g "indel/raw_results/mSigHdp.results"
    message("Looking for analysis.name=", analysis.name)
    toolName <- sub("\\.results", "", basename(analysis.name), fixed = TRUE)
    
    dataset_paths <- list.files(analysis.name, full.names = TRUE)
    for (dataset_path in dataset_paths) {
      # dataset_path, e.g ""indel/raw_results/mSigHdp.results/Moderate"
      # if (!grepl("Realistic", dataset_path)) next
      
      # datasetName <- basename(dataset_path)
      noise.level <- basename(dataset_path)
      if (!noise.level %in% 
...
move_sigpro_files_all_level1_dirs <- function(level1_dirs = level1_dirs) {
  lapply(level1_dirs, move_sigpro_files_all_level1_dirs)
  message("\n=====================")

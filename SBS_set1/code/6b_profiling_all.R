...
topLevelFolder4Data <- "./SBS_set1/input"
topLevelFolder4Run <- "./SBS_set1/raw_results"
folder4Summary <- "./SBS_set1/summary/"
...
# Also summarize toolNamesExt on Realistic data sets
for (toolName in toolNamesExt) {
  for (seedInUse in seedsInUse) {
    run.path <- paste0(topLevelFolder4Run, "/",
                       toolName, ".results/Realistic/",
                       "/seed.", seedInUse)
    load(paste0(run.path, "/code.profile.Rdata"))

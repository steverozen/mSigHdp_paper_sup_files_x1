...
author: "Alvin Ng, Nanhai Jiang, Steve Rozen"
date: "25/01/2022"
output: html_document
---
## Generate stack barcharts for tumors in the PCAWG dataset 
...
This plot originally developed by Alvin Wei Tian Ng for
Alexandrov, L. B., Kim, J., Haradhvala, N. J., Huang, M. N., Tian Ng, A. W., Wu, Y., et al,
 (2020). The repertoire of mutational signatures in human cancer. Nature, 578(7793), 94-101. doi:10.1038/s41586-020-1943-3

### Note:
Please install the dependencies before running the script.
Required packages are on CRAN except mSigAct and PCAWG7.
Set `eval = T` in the next chunk if  you want automatially
...
process.input <- function(x){ ## sample name to grep out 
  ...
  list( counts = list('snv' = s.counts, 
                      'indel' = i.counts, 
                      'dinuc' = d.counts), 
        activity = list( 'snv' = s.act, 
                         'indel' = i.act, 
                         'dinuc' = d.act), 
        sigs = list( 'snv' = s.sigs, 
                     'indel' = i.sigs, 
                     'dinuc' = d.sigs
                     )
        )
...
'#c8afd5',
'#6c409a',
'#fbeeac', # the cream color does not look good on white
'#b15928',
'#b8c8d1',
'#4d7088',
'#b2c5a3',
...
```{r snv}
## adapted from mini's dinuc plot
plot_snvs <- function(counts, mut.type, display.name, class , labels, rect , grid = F, line = F){
    ...
    # barplot
    bp = barplot(counts, main='', xaxt='n', ylim=c(0,ymax), xlab='', yaxt='n', ylab=NA, 
    ...
    if (line == T) axis(1, col='grey60', at = c(1, 225),lwd = 0.5, col.ticks = NA, labels = NA, xpd = NA)
    ...
    if (rect == T) rect(xleft=x.left, ymax*1.01, xright=x.right, ymax*1.15, col=class.col, border=NA, xpd=NA)
    
    # mutation class labels at the top of the figure:
    maj.class.names = c("C>A", "C>G", "C>T", "T>A", "T>C", "T>G")
    
    if (class ==T) text((x.left+x.right)/2, ymax*1.25, labels=maj.class.names, cex=1, xpd=NA)
    
    # x axis label
    if (labels==T) text(bp, -ymax/100, labels=mut.type, cex=0.5, srt=90, adj=1, family='Courier', col='black', xpd=NA)
    
    # sig name
    display.name = gsub('\\.', ' ', display.name)
    #display.name = gsub('Dinucs ', 'Dinucs-', display.name)
...
plot_indels <- function(counts, mut.type, display.name, class , labels, rect, grid = F, line = F){
  num.classes = length(counts)
  ...
  if (line == T)   axis(1, col='grey60', at = c(1, 196),lwd = 0.5, col.ticks = NA, labels = NA, xpd = NA)
...
if (rect == T){
  ...
  text((x.left + x.right)/2, ymax*1.25, labels=category.lab, cex=1, xpd=NA)
  }

  # mutation class labels at the top of the figure:
  maj.class.names = c("1bp\ndeletions", "1bp\ninsertions", ">1bp deletions\nat repeats",
                      ">1bp insertions\nat repeats", "Deletions with\nmicrohomology")
  ...
  if (class ==T) text(class.pos, ymax*1.7, labels=maj.class.names, cex=1, xpd=NA)
...
```{r dinuc}
## mini's dinuc plot
plot_dinucs <- function(counts, mut.type, display.name, class , labels, rect, grid = F, line = F){
  num.classes = length(counts)
...
if (line == T) axis(1, col='grey60', at = c(1, 185),lwd = 0.5, col.ticks = NA, labels = NA, xpd = NA)
  ...
  x.right = bp[c(9,15,24,30,39,45,51,60,69,78)]+0.5
  #x.right = bp[c(9,15,24,30,39,45,51,60,69,78)]+0.5
  if (rect == T) rect(xleft=x.left, ymax*1.01, xright=x.right, ymax*1.15, col=dinuc.class.col, border=NA, xpd=NA)
  
  # mutation class labels at the top of the figure:  
  maj.class.names = c("AC>NN","AT>NN","CC>NN","CG>NN","CT>NN",
...
plot.mutograph  <- function(input, name){
  ...
  #num.dinuc.sigs <- length(input$sigs$dinuc)
  num.dinuc.sigs <- 5
  
  #num.indel.sigs <- length(input$sigs$indel)
  num.indel.sigs <- 5
  
  snv.sig.ind <- as.vector(rbind(rep(3,each= num.snv.sigs), 3+ seq(num.snv.sigs)))
  max.snv.ind <- max(snv.sig.ind) +1
  
  dinuc.sig.ind <- as.vector(rbind(rep(max.snv.ind+2 ,each= num.dinuc.sigs),  max.snv.ind +2+ seq(num.dinuc.sigs)))
  max.dinuc.ind <- max(dinuc.sig.ind)+1
  
  indel.sig.ind <- as.vector(rbind(rep(max.dinuc.ind +2 ,each= num.indel.sigs),  max.dinuc.ind +2 + seq(num.indel.sigs)))
  max.indel.ind <- max(indel.sig.ind)+1

  ## makes a grid of 30 x2, left column is smaller, 0s are empty space
  layout.mat <- matrix(c(0, 1, 
                           0, 1,
                           0, 2,
                           0, 2,
  ...
  ## spectra title
  title(main=colnames(snv.c), line=1.4, cex.lab=1)
  ...
  title(paste0('Cosine similarity : ', round(mSigAct::cossim(snv.prop, recon.prop),3)), cex.main = 1, line= -1 )
  
  ##  snv exposure bar plot 
  bar1  <- barplot(snv.exp, xaxt='n', yaxt='n', 
                   col = as.vector(snv.col.df[match(rev(snv.s),snv.col.df$sig),'color']),
                   space = 5, 
                   border=NA, 
                   xpd = NA)
  ...
  #text(6, bar.pos1 - sum(bar.pos1)*0.02, labels = snv.exp[,1], adj = c(0,0))
  total.snv  <- round(sum(snv.exp))
  
  title(main = paste("SBS Spectrum\n(", total.snv,')\n\nReconstructed\nSpectrum', sep=''), 
        ylab = 'Signature\nContributions',
        line=0.6,
        cex.main = 1.1,
        cex.lab = 0.9 )
  ...
  dinuc.recon  <-  as.matrix(dinuc.sigs[, rev( dinuc.s), drop= F ]) %*% dinuc.exp
  ...
  plot_dinucs(dinuc.recon.prop, 
            NA, NA,
              ...
              rect = F,
              grid = T)
  title(paste0('Cosine similarity : ', round(mSigAct::cossim(dinuc.prop, dinuc.recon.prop),3)), cex.main = 1, line= -1 )
  
  
  ## dinuc exposure 
  bar2  <- barplot(dinuc.exp, xaxt='n',  yaxt='n',
                   col =as.vector(di.col.df[match(rev(dinuc.s), di.col.df$sig),'color']), 
                   space = 5, 
                   border=NA, 
                   xpd = NA)
  ...
  #text(6, bar.pos3 - sum(bar.pos3)*0.015, labels = dinuc.exp[,1], adj = c(0,0))
  
  total.dinuc  <- round(sum(dinuc.exp))
  title(main = paste('DBS Spectrum\n(',total.dinuc, ')\n\nReconstructed\nSpectrum', sep='')
        , ylab="Signature\nContributions", line=0.6, cex.main = 1.1, cex.lab=0.9)
  ...
  ## pad empty plots
  if (length(dinuc.s) < 5){
    for (empty in seq(1, 5-length(dinuc.s))) plot(0,type='n',axes=FALSE,ann=FALSE)
    }
  ...
  title(paste0('Cosine similarity : ', round(mSigAct::cossim(indel.prop, indel.recon.prop),3)), cex.main = 1, line= -1 )
  ...
  #text(6, bar.pos2 - sum(bar.pos2)*0.03, labels = indel.exp[,1], adj = c(0,0))
  
  total.indel  <- round(sum(indel.exp))
  title(main = paste("ID Spectrum\n(", total.indel, ')\n\nReconstructed\nSpectrum', sep=''), 
  ...
  ## pad empty plots
  if (length(indel.s) < 5){
    for (empty in seq(1, 5-length(indel.s))) plot(0,type='n',axes=FALSE,ann=FALSE)
    }
...
output.list  <- lapply("SP116341", process.input)
...
snv.col.df  <- data.frame(sig= rev(snv.sig.col),
                          color = cols[1:length(snv.sig.col)])  
  
di.col.df  <-  data.frame(sig = rev(dinuc.sig.col), 
                          color = cols[seq(1,length(dinuc.sig.col)*2, by=2)])

indel.col.df  <-  data.frame(sig = indel.sig.col,
                             color = cols[seq(1, length(indel.sig.col) *2, by =2)+1])
```

```{r}
plot.mutograph(output.list[[1]], "SP116341")
knitr::include_graphics("SP116341.mutograph.pdf")
```

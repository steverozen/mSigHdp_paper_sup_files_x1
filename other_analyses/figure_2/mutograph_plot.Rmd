...
author: "Alvin Wei Tian Ng, Nanhai Jiang, Steven G. Rozen"
date: "25/01/2022"
...
This plot was originally developed by Alvin Wei Tian Ng for
...
plot_indels <- function(counts, mut.type, display.name, class, labels,
  ...
  if (rect == T) {
    ...
    text((x.left + x.right) / 2, ymax * 1.25, labels = category.lab, 
         cex = 1, xpd = NA)
  }
...
plot.mutograph <- function(input, name) {
  name2 <- sub("::", "_", name)
  png(paste(name2, "mutograph.png", sep = "."), width = 8.2677, 
  ...
  title(paste0("Cosine similarity : ", 
  ...
  ## pad empty plots
  if (length(indel.s) < 5) {
    for (empty in seq(1, 5 - length(indel.s))) plot(0, type = "n", 
...
```{r, out.width="100%", out.height="50%", message=FALSE}
